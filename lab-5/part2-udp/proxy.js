...
server.on("message", (msg, rinfo) => { // Whenever a packet is received, decide randomly whether to forward it or drop it
    total++;
    if (Math.random() > DROP_RATE) {
        console.log(
            `[${total}] Forwarding packet from ${rinfo.address}:${rinfo.port} → localhost:${DEST_PORT}`
        );
        server.send(msg, DEST_PORT, "localhost"); // Forward the packet to the destination port
    } else {
        dropped++;
